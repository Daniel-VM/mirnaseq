# nf-core/mirnaseq: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/mirnaseq/usage](https://nf-co.re/mirnaseq/usage)
...
```console
nextflow run nf-core/mirnaseq --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/mirnaseq
```
...
First, go to the [nf-core/mirnaseq releases page](https://github.com/nf-core/mirnaseq/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
